...
if ($action === 'get') {
    ...
    $stmt = $pdo->prepare("SELECT cl.name as client_name,cl.phone as client_phone, c.* FROM caisse c LEFT JOIN transactions t ON c.transaction_id = t.id LEFT JOIN clients cl ON t.client_id = cl.id AND cl.deleted_at IS NULL WHERE c.user_id = ?");
